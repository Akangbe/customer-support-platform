...
**WhatsApp locally:** `application.yml` ships working local-dev defaults for `app.whatsapp.*` (verify-token, app secret, credential-encryption key) so tests and local runs work with no extra setup. `-dev`/`-prod` profiles require the real `WHATSAPP_VERIFY_TOKEN` / `WHATSAPP_APP_SECRET` / `WHATSAPP_CREDENTIAL_ENCRYPTION_KEY` env vars — no defaults there. The inbound/outbound processors (`whatsapp-domain.md` §4, §6) run when work arrives — a stored webhook, a queued agent message, a retry falling due — plus an hourly backstop sweep (`WhatsAppWorkScheduler`); they never poll on a short timer, because that kept the Neon database awake around the clock. Integration tests disable them (`app.scheduling.enabled=false`) and call the poller beans directly instead, so a poller never races a test's own assertions.

**Storage locally:** `application.yml` ships MinIO-compatible local defaults for `app.storage.*` (endpoint `http://localhost:9000`, `minioadmin`/`minioadmin`) — run a local MinIO container (`docker run -p 9000:9000 -e MINIO_ROOT_USER=minioadmin -e MINIO_ROOT_PASSWORD=minioadmin minio/minio server /data`) and create the `attachments-local` bucket to exercise attachment upload/download outside tests. Tests don't need this — `AbstractStorageIntegrationTest`/`AbstractWhatsAppMediaIntegrationTest` spin up their own MinIO Testcontainer and create their bucket automatically. `-dev`/`-prod` profiles require the real `STORAGE_ENDPOINT`/`STORAGE_BUCKET`/`STORAGE_ACCESS_KEY`/`STORAGE_SECRET_KEY` env vars (Cloudflare R2 in those environments) — no defaults there.
